...
export default function Upstu() {
  ...
  const getData = async () => {
    let res = await axios.get(`${env.apiurl}/users/getUpStuData/${params.id}`);
  ...
  const handleSubmit = async () => {
    let res = await axios.put(`${env.apiurl}/users/updateStu/${params.id}`, {
      firstName,
      lastName,
      mobileNumber,
      role,
  ...
  const handleBack = () => {
    navigate("/CordinatorDash/AddStudent");
  };
